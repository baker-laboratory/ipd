...
template <class F> F epsilon2() { return std::sqrt(std::numeric_limits<F>::epsilon()); }
...
public:
  ...
  Sphere(Vec3<F> c, F r) : cen(c), rad(r) {}
  Sphere(Vec3<F> O) : cen(O), rad(epsilon2<F>()) {}
  Sphere(Vec3<F> O, Vec3<F> A) {
    Vec3<F> a = A - O;
    Vec3<F> o = 0.5 * a;
    rad = o.norm() + epsilon2<F>();
    cen = O + o;
  }
  Sphere(Vec3<F> O, Vec3<F> A, Vec3<F> B) {
    Vec3<F> a = A - O, b = B - O;
    F det_2 = 2.0 * ((a.cross(b)).dot(a.cross(b)));
    Vec3<F> o = (b.dot(b) * ((a.cross(b)).cross(a)) + a.dot(a) * (b.cross(a.cross(b)))) / det_2;
    rad = o.norm() + epsilon2<F>();
    cen = O + o;
  }
  Sphere(Vec3<F> O, Vec3<F> A, Vec3<F> B, Vec3<F> C) {
    Vec3<F> a = A - O, b = B - O, c = C - O;
    Mat33<F> cols;
    cols.col(0) = a;
    cols.col(1) = b;
    cols.col(2) = c;
    F det_2 = 2.0 * Mat33<F>(cols).determinant();
    Vec3<F> o = (c.dot(c) * a.cross(b) + b.dot(b) * c.cross(a) + a.dot(a) * b.cross(c)) / det_2;
    rad = o.norm() + epsilon2<F>();
    cen = O + o;
  }
  ...
  F signdis(Vec3<F> P) const { return (cen - P).norm() - rad; }
  F signdis2(Vec3<F> P) const { // NOT square of signdis!
  ...
  bool contains(Vec3<F> pt) const { return (cen - pt).squaredNorm() < rad * rad; }
...
// template <class F> Sphere<F> operator*(X3<F> x, Sphere<F> s) { return Sphere<F>(x * s.cen, s.rad); }

template <class Scalar> std::ostream &operator<<(std::ostream &out, Sphere<Scalar> const &s) {
  out << "Sphere( " << s.cen.transpose() << ", " << s.rad << ")";
  return out;
}

template <class Ary>
auto welzl_bounding_sphere_impl(Ary const &points,
                                size_t index,
  ...
  Sph smallestSphere = welzl_bounding_sphere_impl(points, index, sos, numsos); // (*)
...
template <bool range = false, class Ary> auto welzl_bounding_sphere(Ary const &points) {
  using Pt = typename Ary::value_type;
...
template <bool range = false, class Ary> auto central_bounding_sphere(Ary const &points) {
  using Pt = typename Ary::value_type;
  using Scalar = typename Pt::Scalar;
  using Sph = Sphere<Scalar>;
