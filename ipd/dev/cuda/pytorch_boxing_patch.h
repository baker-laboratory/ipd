...
struct is_tuple_of_mutable_tensor_refs<
    T, std::enable_if_t<guts::is_instantiation_of<std::tuple, T>::value, void>>
    : guts::typelist::all<is_mutable_tensor_ref,
                          guts::typelist::from_tuple_t<T>> {};
...
template <typename T>
using can_box =
    std::disjunction<std::is_constructible<IValue, std::decay_t<T>>,
                     // TensorOptions are not directly constructible into
                     // IValue, but torch::jit::push knows how to handle them
                     std::is_same<TensorOptions, std::decay_t<T>>>;

template <typename... Ts> using can_box_all = std::conjunction<can_box<Ts>...>;

// an unboxable result is one that can be extracted from an IValue
template <typename T>
using can_unbox = std::conjunction<std::disjunction<has_ivalue_to<T>,
                                                    // void returns are ok
                                                    std::is_same<void, T>>,
...
//
template <class... Args> torch::jit::Stack boxArgs(Args... args) {
...
template <class T> static inline constexpr size_t boxed_size_one() {
  static_assert(
...
// be kept in sync.
template <> inline constexpr size_t boxed_size_one<c10::TensorOptions>() {
  return 4;
}

// NOTE: this could probably be simplified with C++17 fold expressions.
template <typename...> struct BoxedSize : std::integral_constant<size_t, 0> {};
template <class T, class... Args>
struct BoxedSize<T, Args...>
    : std::integral_constant<size_t, boxed_size_one<T>() +
                                         BoxedSize<Args...>::value> {};

template <class... Args> static inline constexpr size_t boxed_size() {
...
template <typename T>
C10_ALWAYS_INLINE_UNLESS_MOBILE void boxToStack(IValueAlignedStorage *dest,
                                                T &arg, int &lastIdx) {
...
C10_ALWAYS_INLINE_UNLESS_MOBILE void boxArgsToStack(IValueAlignedStorage *dest,
                                                    int &lastIdx, T &arg,
                                                    Args &...args) {
...
//
template <class Result> struct PopResult final {
...
template <class... Types> struct PopResult<std::tuple<Types...>> final {
  ...
  static Result call(Stack &stack) {
    ...
    TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
        stack.size() == RetCount, "Boxed kernel was expected to return ",
        RetCount, " values on the stack, ", "but instead pushed ", stack.size(),
        " values.");
...
private:
  ...
  static Result pop_to_tuple_impl(Stack &stack,
...
//
template <class FuncType, class Enable = void> struct BoxedKernelWrapper {
  ...
  static_assert(sizeof(FuncType) != sizeof(FuncType),
                "Function signature contains one or more unsupported parameter "
                "and/or return types. "
                "Look for a nearby error like "
                "\"'call' is not a member of "
                "'c10::impl::BoxedKernelWrapper<(your function type), void>'\" "
...
struct BoxedKernelWrapper<
    Result(Args...),
    std::enable_if_t<can_box_all<Args...>::value && can_unbox<Result>::value &&
                         !is_tuple_of_mutable_tensor_refs<Result>::value,
                     void>> {
  static Result call(const BoxedKernel &boxed_kernel_func,
...
struct BoxedKernelWrapper<
  ...
  static at::Tensor &call(const BoxedKernel &boxed_kernel_func,
                          const OperatorHandle &opHandle,
                          DispatchKeySet dispatchKeySet, at::Tensor &outArg,
                          OtherArgs... otherArgs) {
...
struct BoxedKernelWrapper<
    ...
    std::enable_if_t<can_box_all<OtherArgs...>::value, void>> {
  static const at::Tensor &call(const BoxedKernel &boxed_kernel_func,
                                const OperatorHandle &opHandle,
...
struct BoxedKernelWrapper<
    ...
    std::enable_if_t<can_box_all<FirstArg, RestArgs...>::value
                         // this skips over in-place kernels with a non-const
                         // Tensor arg at the front, so those can unambiguously
                         // trigger the preceding specialization.
                         && !is_mutable_tensor_ref<FirstArg>::value,
                     void>> {
  static at::Tensor &call(const BoxedKernel &boxed_kernel_func,
                          const OperatorHandle &opHandle,
                          DispatchKeySet dispatchKeySet, FirstArg firstArg,
                          RestArgs... restArgs) {
...
struct BoxedKernelWrapper<
    Result(Args...),
    std::enable_if_t<can_box_all<Args...>::value &&
                         is_tuple_of_mutable_tensor_refs<Result>::value,
                     void>> {
  static Result call(const BoxedKernel &boxed_kernel_func,
                     const OperatorHandle &opHandle,
    ...
    TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
        stack.size() == RetCount, "Boxed kernel was expected to return ",
        RetCount, " values on the stack, ", "but instead returned ",
        stack.size(), " values.");
