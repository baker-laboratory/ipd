...
def get_pseudo_highT(opt):
	pseudo_gen = PseudoSymmGenerator(h=opt.H_K[0], k=opt.H_K[1], sphericalFrac=opt.sphericalFrac)
...
class PseudoSymmGenerator:
	# from Frank DiMaio
    def __init__(self, h, k, sphericalFrac=0.0):
        # 0) h=1,k=0 ref
        ...
        # 1) get points
        ico_samples = self.gen_ico_samples(h,k,sphericalFrac)
    ...
    def gen_ico_samples(self, H, K, sphericalFrac):
        ...
        for ii,fi in enumerate(TRIS):
            ...
            for h in range(-(H+K),(H+K+1)):
                for k in range(-(H+K),(H+K+1)):  
                    ...
                    w = 1 - u - v
                    if (
                        (u>-1e-6 and v>-1e-6 and w>-1e-6) 
                    ): # in the triangle
                        if sphericalFrac!=0.0:
                            newpointS = newpoint / np.linalg.norm( newpoint )
                            newpoint = sphericalFrac*newpointS + (1-sphericalFrac)*newpoint
                        ico.append( newpoint )
