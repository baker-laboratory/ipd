...
def symm_subunit_matrix(symmid, symopt=None):
    if (symmid[0]=='C'):
        ...
        metasymm = (
            [torch.arange(nsub)],
            [min(nsub, symopt.max_nsub if symopt else min(3,nsub))]
        )
        ...
        metasymm = (
            ...
            [min(nsub, symopt.max_nsub if symopt else min(3,nsub)), 2]
        )
        #metasymm = (
        #    [torch.arange(2*nsub)],
        #    [min(2*nsub,5)]
        ...
        metasymm = (
            [torch.arange(12)],
            [min(len(Rs), symopt.max_nsub if symopt else 6)]
        )
...
def update_symm_Rs(xyz, Lasu, symmsub, allsymmRs, symopt):
    def dist_error_comp(R0,T0,xyz,fittscale):
        ...
        Tcorr = torch.einsum('ij,brj->bri', R0, xyz[:,:Lasu]-Tcom) + Tcom + fittscale*T0[None,None,:] # LT Rotated coordinates of first ASU by learned R0, then translated by learned T0
    ...
    if symopt.fit:
        with torch.enable_grad():
            ...
            Q0 = Q0.detach()
            T0 = T0.detach()
            xyz = torch.einsum('ij,braj->brai', Q2R(Q0), xyz[:,:Lasu]-Tcom) +Tcom + symopt.fittscale*T0[None,None,:]
