...
def sym_com(sym, xyz, Lasu, **kw):
    ...
    # ic(xyz.shape)
    # ic(com.shape)
    return th.einsum('sij,j->si', sym.allsymmRs, com)

def asu_to_best_frame_if_necessary(sym, xyz, Lasu, asu_to_best_frame=None, disable_all_fitting=None, **kw):
    ...
    if asu_to_best_frame and not disable_all_fitting:
        ...
        isub = th.argmin(d2)
        xyz[:Lasu] = th.einsum('ij,raj->rai', sym.allsymmRs[isub], xyz[:Lasu])
    return xyz
...
def set_motif_placement_if_necessary(sym, xyz, disable_all_fitting=None, **kw):
    ...
    if sym.opt.motif_position == "AB" and sym.opt.motif_fit:
        ...
        # import willutil as wu
        # wu.showme(gpbb.reshape(-1,3), 'gpbb', spheres=0.3)
        # wu.showme(bb.reshape(-1,3), 'bb', spheres=0.3)
        # ic(gpbb.shape)
        # ic(bb.shape)
        ...
        # ic(rms2)
        # wu.showme(y, sphere=0.5, name='fitcagp', col=(1,0,0))
        # wu.showme(x, sphere=0.5, name='fitca', col=(1,0.6 ,0))
        # wu.showme(fit, sphere=0.5, name='fitca', col=(1,1 ,0))

        # assert th.allclose(gpbb, xyz[sym.idx.gpca, 1])
        # wu.showme(xyz[~mask, 1], name='prot0', sphere=0.3)
        ...
        # import willutil as wu
        # wu.showme(xyz[:,1], name='all', sphere=0.4)
        # ic(th.abs(x-y))
