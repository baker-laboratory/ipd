...
template <uint64_t P, uint64_t Q> struct X_P_Q {
  template <uint64_t l, uint64_t MOCK> struct XPQ_IMPL {
    static const uint64_t VAL = XPQ_IMPL<l - 1, MOCK>::VAL + SAFE_LSHIFT<uint64_t, 1, l * Q>::VAL;
  };
...
template <uint64_t I_PLUS_1, uint64_t D> struct UMULT_I_D {
  static const uint64_t VAL = X_P_Q<D, (D - 1) * POW<D, I_PLUS_1 - 1>::VAL>::VAL;
};
template <uint64_t ITER, uint64_t D> struct DMULT_I_D {
  static const uint64_t VAL = X_P_Q<D, POW<D - 1, DILATION_ROUNDS<D>::VAL - ITER + 1>::VAL>::VAL;
};
...
template <uint64_t D> struct UNDILATE {
  template <uint64_t ITER, uint64_t MOCK> struct UNDILATE_IMPL {
    static inline uint64_t eval(uint64_t val) {
      return (UNDILATE_IMPL<ITER - 1, MOCK>::eval(val) * UMULT_I_D<ITER, D>::VAL) & UMASK_I_D<ITER, D>::VAL;
    }
  };
  template <uint64_t MOCK> struct UNDILATE_IMPL<1, MOCK> {
    static inline uint64_t eval(uint64_t val) { return (val * UMULT_I_D<1, D>::VAL) & UMASK_I_D<1, D>::VAL; }
  };
  static inline uint64_t eval(uint64_t val) {
...
template <uint64_t D> struct DILATE {
  ...
  template <uint64_t MOCK> struct DILATE_IMPL<1, MOCK> {
    static inline uint64_t eval(uint64_t val) { return (val * DMULT_I_D<1, D>::VAL) & DMASK_I_D<1, D>::VAL; }
  };
  static inline uint64_t eval(uint64_t val) { return DILATE_IMPL<DILATION_ROUNDS<D>::VAL, 0>::eval(val); };
};
...
template <> struct DILATE<2> {
  template <uint64_t ITER> struct shiftval {
    static const uint64_t VAL = SAFE_LSHIFT<uint64_t, 1, DILATION_ROUNDS<2>::VAL - ITER>::VAL;
  };
  template <uint64_t ITER, uint64_t MOCK> struct DILATE_IMPL {
    static inline uint64_t eval(uint64_t val) {
  ...
  static inline uint64_t eval(uint64_t val) { return DILATE_IMPL<DILATION_ROUNDS<2>::VAL, 0>::eval(val); };
};
} // namespace impl
