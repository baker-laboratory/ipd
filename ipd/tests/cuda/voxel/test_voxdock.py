...
def test_voxdock_ab(timer=ipd.dev.Timer()):
    ...
    # rb = ipd.cuda.voxel.VoxRB(xyz[:128], resl=1, func=ipd.cuda.ContactFunc())

    rb = ipd.cuda.voxel.VoxRB(xyz,
                         resl=1,
...
def test_voxdock_c3():
    ...
    rb = ipd.cuda.voxel.VoxRB(xyz, resl=1, func=ipd.cuda.ContactFunc(1000, -1, 4, 5, 9, 10))
    # ipd.showme(rb, col=(1, 1, 1), name='ref', sphere=2)
    N = 100_000
    Ntop = 1000
...
def voxdock_cage(xyz,
    ...
    rb = ipd.cuda.voxel.VoxRB(xyz, resl=resl, func=ipd.cuda.ContactFunc(1000, -1, 5, 6, 9, 10))
