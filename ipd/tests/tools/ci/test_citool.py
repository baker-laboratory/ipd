...
def test_clitool_pytest():
    ...
    assert out.strip(
    ) == 'cd TESTDIR && OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 PYTHONPATH=. EXE --benchmark-disable --disable-warnings --durations=10 > pytest_ipd_ci_test_run.log'

    out = runipd("ci tests pytest --cmdonly --exe $exe --slurm --tee --gpu a4000")
    assert out.strip(
    ) == 'cd TESTDIR && OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 PYTHONPATH=. EXE --benchmark-disable --disable-warnings --durations=10 2>&1 | tee pytest_ipd_ci_test_run.log'

    out = runipd(
        "ci tests pytest --cmdonly --exe $exe --slurm --parallel 4 --tee --which 'test_call_speed test_loss_grad'")
    print(out)
    assert out.strip(
    ) == """cd TESTDIR && OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 PYTHONPATH=. EXE -k "not test_call_speed and not test_loss_grad" -n 4 --benchmark-disable --disable-warnings --durations=10 2>&1 | tee pytest_ipd_ci_test_run.log.par.log
cd TESTDIR && OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 PYTHONPATH=. EXE -k "test_call_speed or test_loss_grad" --benchmark-disable --disable-warnings --durations=10 2>&1 | tee pytest_ipd_ci_test_run.log.nopar.log"""
