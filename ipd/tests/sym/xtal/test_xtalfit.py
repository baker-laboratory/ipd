...
def test_xtalfit_I213():
    ...
    ipd.icv(rorig)  # type: ignore
    ipd.icv(rfit)  # type: ignore
    vals = list()
    for i in range(10):
    ...
    vals = np.array(vals)
    ipd.icv(np.min(vals), np.mean(vals))  # type: ignore
...
def DISABLED_test_xtalfit_I213_bk():
    ...
    ipd.icv(cen2, cen3)  # type: ignore
    cell = (cen2[0] / 2 + cen2[1]) / 2 * 4
    ipd.icv(cell)  # type: ignore
