...
struct eigen_random_device {
  ...
  static constexpr int Entropy =
      meta_floor_log2<(unsigned int)(RAND_MAX) + 1>::value;
  static constexpr ReturnType Highest = RAND_MAX;
  static EIGEN_DEVICE_FUNC inline ReturnType run() { return std::rand(); }
};

// Fill a built-in unsigned integer with numRandomBits beginning with the least
// significant bit
template <typename Scalar> struct random_bits_impl {
  EIGEN_STATIC_ASSERT(std::is_unsigned<Scalar>::value,
...
// uses double as the implementation with a mantissa with a size equal to either
// the target scalar's mantissa or that of double, whichever is smaller
template <typename Scalar> struct random_float_impl<Scalar, false> {
  ...
  static EIGEN_DEVICE_FUNC inline Scalar run(int numRandomBits) {
    eigen_assert(numRandomBits >= 0 && numRandomBits <= mantissaBits());
    Scalar result =
        static_cast<Scalar>(random_float_impl<double>::run(numRandomBits));
    return result;
  }
};
...
template <> struct random_longdouble_impl<false> {
  ...
  static EIGEN_DEVICE_FUNC inline long double run(int numRandomBits) {
    return static_cast<double>(random_float_impl<double>::run(numRandomBits));
  }
};
template <> struct random_float_impl<long double> : random_longdouble_impl<> {};
...
template <typename Scalar> struct random_int_impl<Scalar, false, true> {
  static constexpr int kTotalBits = sizeof(Scalar) * CHAR_BIT;
  static EIGEN_DEVICE_FUNC inline Scalar run(const Scalar &x, const Scalar &y) {
    if (y <= x)
      return x;
    ...
    if (range == NumTraits<Scalar>::highest())
      return run();
    ...
    do {
      ...
      // if the random draw is outside [0, range), try again (rejection
      // sampling) in the worst-case scenario, the probability of rejection is:
      // 1/2 - 1/2^numRandomBits < 50%
...
template <typename Scalar> struct random_int_impl<Scalar, true, true> {
  static constexpr int kTotalBits = sizeof(Scalar) * CHAR_BIT;
  ...
  static EIGEN_DEVICE_FUNC inline Scalar run(const Scalar &x, const Scalar &y) {
    ...
    // Avoid overflow in the case where `x` is negative and there is a large
    // range so `randomBits` would also be negative if cast to `Scalar` first.
    Scalar result = static_cast<Scalar>(static_cast<BitsType>(x) + randomBits);
    return result;
  }
  static EIGEN_DEVICE_FUNC inline Scalar run() {
    return static_cast<Scalar>(getRandomBits<BitsType>(kTotalBits));
  }
};
...
struct random_int_impl<Scalar, IsSigned, false> {
  static EIGEN_DEVICE_FUNC inline Scalar run(const Scalar &, const Scalar &) {
    return run();
  }
  static EIGEN_DEVICE_FUNC inline Scalar run() {
...
template <> struct random_impl<bool> {
  ...
  static EIGEN_DEVICE_FUNC inline bool run() {
    return getRandomBits<unsigned>(1) ? true : false;
  }
};
...
template <typename Scalar> struct random_default_impl<Scalar, true, false> {
  ...
  static EIGEN_DEVICE_FUNC inline Scalar run(const Scalar &x, const Scalar &y,
                                             int numRandomBits) {
    return Scalar(Impl::run(x.real(), y.real(), numRandomBits),
                  Impl::run(x.imag(), y.imag(), numRandomBits));
  }
  static EIGEN_DEVICE_FUNC inline Scalar run(const Scalar &x, const Scalar &y) {
